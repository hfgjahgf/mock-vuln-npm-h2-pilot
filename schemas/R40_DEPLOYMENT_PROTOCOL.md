...
> **为什么 `dataset_metadata.json` 不整份比对**:其中**两个键记录的是机器而不是数据** ——
> `generated_at_utc`(时间戳)与 **`input_dir`(读取输入的绝对路径)**。
> 拿整份比会因为一个时间戳和一条路径而失败, 而「把它们删掉让它通过」就是为了让检查通过
> 去改被检查的东西。故**按块比对**:`section_sha256` / `inputs` / `counts` 三块必须相同,
> 被排除的两个键**逐一列名**。
>
> **更正(第三次预跑)**:此处与工作流注释原先都写着「`generated_at_utc` 是唯一不同的键」。
> 那句话对**同一台机器上的两次重建**成立, 对 runner **不成立** —— `input_dir` 也不同。
> 它没有造成任何后果(E1 比的是段, 不是这个文件), **但它是一句主张, 而且是错的**。
